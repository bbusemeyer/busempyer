...
from numpy import array,asarray,diag,zeros,empty,dot,eye
ANG = 1/1.889725989 
...
# TODO decide units.
class VolData:
  def __init__(self,data=None,voxel=None,positions=None,latvecs=None,origin=(0.0,0.0,0.0),meta=None):
...
def compute_pyscf_points(mol,voxel,npoints,coeff):
  ...
  coords = make_grid(voxel,npoints)
  ngrids = coords.shape[0]

  blksize = min(8000, ngrids)
  orb_on_grid = empty(ngrids)
  for ip0, ip1 in lib.prange(0, ngrids, blksize):
    ao = mol.eval_gto(GTOval, coords[ip0:ip1])
    orb_on_grid[ip0:ip1] = dot(ao, coeff)

  return orb_on_grid.reshape(npoints)

def make_grid(voxel=eye(3),npoints=(10,10,10)):
  ''' Make a grid at npoints multiples of voxvecs.'''
  grid = empty((npoints[0],npoints[1],npoints[2],3))

  for i in range(npoints[0]):
    ipart = i*voxel[0]
    for j in range(npoints[1]):
      ijpart = ipart + j*voxel[1]
      for k in range(npoints[2]):
        grid[i,j,k,:] = k*voxel[2] + ijpart

  return grid.reshape(npoints[0]*npoints[1]*npoints[2],3)
